...
import me.xiba.startlearnmvvm.difftype.event.DiffTypeItemAEvent;
 ...
 * @Description: 数据类型为TypeA的列表
 */
public class DiffTypeAFragment extends Fragment {
    ...
    //数据为TypeA时的事件处理
    private DiffTypeItemAEvent mEvent;
    ...
    public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
        ...
        mEvent = new DiffTypeItemAEvent();

        //设定ViewModel，布局文件，事件处理和item布局所对应的ViewModel
        mAdapter = new BindingRecyclerAdapter<TypeA, DiffTypeItemAVM>(BR.viewModel,
                R.layout.item_diff_type,
                BR.event,
                mEvent,
                DiffTypeItemAVM.class);
    ...
    public DiffTypeVM getViewModel(){
        if (mViewModel == null) {
            mViewModel = ViewModelProviders.of(this, new DiffTypeViewModelFactory(TypeA.class)).get(DiffTypeVM.class);
